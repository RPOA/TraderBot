...
class AlertWatcher:
    ...
    def start(self) -> bool:
        try:
            ...
            logger.error("Watcher startup failed: %s", exc)
            return False
    ...
    def _start_health_monitor(self) -> None:
        self.health_monitor = HealthMonitor(self.driver, config.TRADINGVIEW_URL)
        if self.health_monitor.start():
    ...
    def snapshot(self) -> dict[str, Any]:
        health = self.health_monitor.status.to_dict() if self.health_monitor else {"is_healthy": False, "issues": []}
        return {
